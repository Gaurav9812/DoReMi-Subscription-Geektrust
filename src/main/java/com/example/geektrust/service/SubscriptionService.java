...
public class SubscriptionService {
    ...
    public void addSubscription(String category, String plan) {
        try {
            ...
            if(subscription != null) throw new AddSubscriptionFailedException("DUPLICATE_CATEGORY");
            Subscription subscription1 = SubscriptionRegistry.getPlan(category, plan);
            user.addSubscription(category, plan, subscription1);
            System.out.println("RENEWAL_REMINDER"+" "+subscription1.getCategory()+" "+ getReminderDate(user.getSubscriptionStart(), subscription1.getMonth()));
        } catch (InvalidDateException e) {
    ...
    public void addTopUp(String topUpPlan, int months) {
        try {
            if(user.getSubscriptions() == null) throw  new AddTopUpFailedException("SUBSCRIPTIONS_NOT_FOUND");
            if(user.getTopUp() != null) throw  new AddTopUpFailedException("DUPLICATE_TOPUP");
    ...
    public void printRenewalDetails() {
        try {
            ...
            System.out.println("RENEWAL_AMOUNT " +totalPrice);
        } catch (AddTopUpFailedException e) {
            System.out.println(e.getMessage());
        }
